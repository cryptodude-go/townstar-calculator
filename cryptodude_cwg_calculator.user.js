...
// @version      0.0.1118a
// @description  CWG Calculator for Common World Ground is an specific build calculator with the option to write current production per hour and improve your build.
...
(function() {
    ...
    // Create a div element for the inline calculator
    const inlineCalculator = document.createElement('div');
    inlineCalculator.id = 'cgcalculator-inline-calculator';
    inlineCalculator.style.width = '400px';
    inlineCalculator.style.height = '500px';
    inlineCalculator.style.overflow = 'none';
    inlineCalculator.style.position = 'fixed';
    inlineCalculator.style.top = '50px'; // Adjust the initial position as needed
    inlineCalculator.style.left = '50px'; // Adjust the initial position as needed
    inlineCalculator.style.display = 'none'; // Initially hide the calculator
    inlineCalculator.style.borderRadius = '15px'; // Add rounded corners

    // Create a title bar for the calculator
    const titleBar = document.createElement('div');
    titleBar.id = 'cgcalculator-title-bar';
    titleBar.innerHTML = `<img src="https://icons.iconarchive.com/icons/froyoshark/enkel/24/Calculator-icon.png" width="24" height="24">
    <span style="padding-left:5px;">${GM_info.script.name} vers. ${GM_info.script.version}</span>`;
    titleBar.style.backgroundColor = '#ff9600'; // Background color
    titleBar.style.color = '#fff'; // Text color
    titleBar.style.padding = '10px';
    titleBar.style.cursor = 'move';
    titleBar.style.fontSize = '14px'; // Font size
    titleBar.style.textAlign = 'center'; // Text alignment
    titleBar.style.fontWeight = 'bold'; // Font weight
    titleBar.style.display = 'flex';
    titleBar.style.alignItems = 'center';
    inlineCalculator.appendChild(titleBar);

    // Create a div for the inline content
    const inlineContent = document.createElement('div');
    inlineContent.id = 'cgcalculator-inline';
    inlineCalculator.appendChild(inlineContent);

    // Create a div element for the inline calculator
    const layoutCalculator = document.createElement('div');
    layoutCalculator.id = 'layout-calculator';
    layoutCalculator.className = 'costs fs-5';
    layoutCalculator.style.border = '5px solid #757575';
    layoutCalculator.style.backgroundColor = '#757575';
    layoutCalculator.style.color = '#fff';

// Create a div element for the v2calc
const v2calc = document.createElement('div');
v2calc.className = 'v2calc';

// Create a div element for the craft calculator main
const craftCalculatorMain = document.createElement('div');
craftCalculatorMain.id = 'craft_calculator_main';

// Create a div element for the craft container
const craftContainer = document.createElement('div');
craftContainer.id = 'craft_container';

// Create a div element for the selectCRAFTs dropdown
const selectCRAFTs = document.createElement('div');
selectCRAFTs.id = 'selectCRAFTs';
selectCRAFTs.className = 'mb-2';

// Create a select element for the selectCRAFTs dropdown
const selectCRAFTsSelect = document.createElement('select');
selectCRAFTsSelect.id = 'selectCRAFTs-select';
selectCRAFTsSelect.addEventListener('change', DisplayCraftCalculation);
...
selectCRAFTsSelect.appendChild(selectCRAFTsOption);

// Append the select element to the selectCRAFTs div
selectCRAFTs.appendChild(selectCRAFTsSelect);

// Create a div element for the selectCRAFTs-QTY
const selectCRAFTsQty = document.createElement('div');
selectCRAFTsQty.id = 'selectCRAFTs-QTY';
selectCRAFTsQty.className = 'mb-3';

// Create an input element for the Amount
const selectCRAFTsQtyInput = document.createElement('input');
selectCRAFTsQtyInput.id = 'selectCRAFTs-QTY-input';
selectCRAFTsQtyInput.type = 'number';
selectCRAFTsQtyInput.min = '1';
selectCRAFTsQtyInput.value = '1';
selectCRAFTsQtyInput.max = '9999';
selectCRAFTsQtyInput.step = '1';

// Create a label element for the Amount
const selectCRAFTsQtyLabel = document.createElement('label');
selectCRAFTsQtyLabel.htmlFor = 'selectCRAFTs-QTY';
selectCRAFTsQtyLabel.textContent = ' / Hour';

// Append the input and label elements to selectCRAFTsQty
selectCRAFTsQty.appendChild(selectCRAFTsQtyInput);
selectCRAFTsQty.appendChild(selectCRAFTsQtyLabel);

// Create a div element for craft_left
const craftLeft = document.createElement('div');
craftLeft.className = 'craft_left';
craftLeft.textContent = 'NEEDED QUANTITY/CRAFT NAME/YOUR QUANTITY';

// Create a div element for craft_content
const craftContent = document.createElement('div');
craftContent.id = 'craft_content';
craftContent.style.overflowY = 'scroll';

// Append all the elements in the desired order to inlineContent
inlineContent.appendChild(layoutCalculator);

// Append all the elements in the desired order
layoutCalculator.appendChild(v2calc);
v2calc.appendChild(craftCalculatorMain);
craftCalculatorMain.appendChild(craftContainer);
craftContainer.appendChild(selectCRAFTs);
craftContainer.appendChild(selectCRAFTsQty);
craftContainer.appendChild(craftLeft);
craftContainer.appendChild(craftContent);

    // Create a Font Awesome icon for toggling the calculator
    const toggleButton = document.createElement('i');
    toggleButton.classList.add('fa', 'fa-calculator', 'fa-2x');
    toggleButton.id = 'cgcalculator-toggle-button';
    ...
    // Add the inline calculator and toggle button to the document body
    document.body.appendChild(inlineCalculator);
    document.body.appendChild(toggleButton);


    // Add styles for light/dark mode
    GM_addStyle(`
        .switch {
            position: relative;
        ...
        body.dark-mode #cgcalculator-inline-calculator {
            background-color: #333;
            color: white;
        }
        body #cgcalculator-inline-calculator {
    ...
    #donation-info-text {color:white;}
    `);
    ...
    // Function to toggle the visibility of the inline calculator
    function toggleCalculator() {
        isCalculatorVisible = !isCalculatorVisible;
        inlineCalculator.style.display = isCalculatorVisible ? 'block' : 'none';
    }

    // Make the inline calculator draggable
    let offsetX, offsetY;
    ...
    function startDragging(e) {
        isDragging = true;
        offsetX = e.clientX - inlineCalculator.getBoundingClientRect().left;
        offsetY = e.clientY - inlineCalculator.getBoundingClientRect().top;
        window.addEventListener('mousemove', dragCalculator, false);
        window.addEventListener('mouseup', stopDragging, false);
    }

    function dragCalculator(e) {
        if (isDragging) {
            inlineCalculator.style.left = e.clientX - offsetX + 'px';
            inlineCalculator.style.top = e.clientY - offsetY + 'px';
        }
    }

    function stopDragging() {
        isDragging = false;
        window.removeEventListener('mousemove', dragCalculator, false);
    ...
    // Add keyboard event listener to toggle the calculator with the "c" key (case-insensitive)
...
function DisplayCalcInfo() {
  ...
  document.querySelector('#craft_calculator_main').style.display = 'block';
}
    ...
    if (requirementName.text().length > 20) {
        ...
        currentValueInput.on('input', function (event) {
            ...
            if (numericValue === '') {
                ...
                if (result < 0) {
                    calcoutput.text(`Missing ${result.toFixed(0)}`).css({
                        color: 'red',
                    });
                } else {
...
// Create a div element for the donation message
const donationMessage = document.createElement('div');
const htmlContent = document.createElement('span');
htmlContent.innerHTML = 'Donate Gala ETH: <b>0xA273633fC817DCa0d538EDa234654D47ef7B7903</b>';
donationMessage.appendChild(htmlContent);
donationMessage.style.fontSize = '12px'; // Add some margin for spacing
donationMessage.style.color = 'white';

// Append the donation message div as the last child of cgcalculator-inline-calculator
inlineCalculator.appendChild(donationMessage);
